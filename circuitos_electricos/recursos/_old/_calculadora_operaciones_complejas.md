...
::: {.card-module}
### 🧮 Calculadora de Operaciones entre Dos Fasores {.unnumbered}

```{=html}
<!-- Entradas de los dos operandos -->
<div class="row g-3">
  <div class="col-md-6">
    <div class="p-3 rounded-3 border h-100" style="border-left: 4px solid #0284c7 !important;">
      <strong class="text-primary">Operando <span style="font-family:'JetBrains Mono',monospace;">Z&#8321;</span></strong>
      <div class="row g-2 mt-1">
        <div class="col-6">
          <label class="form-label mb-1 small">Real <em>x</em>&#8321;</label>
          <input id="op1Real" type="number" class="form-control" value="4" step="0.5" oninput="calcularOperacion()">
        </div>
        <div class="col-6">
          <label class="form-label mb-1 small">Imaginaria <em>y</em>&#8321;</label>
          <input id="op1Imag" type="number" class="form-control" value="3" step="0.5" oninput="calcularOperacion()">
        </div>
      </div>
      <div id="op1Polar" class="small text-muted mt-2" style="font-family:'JetBrains Mono',monospace;"></div>
    </div>
  </div>

  <div class="col-md-6">
    <div class="p-3 rounded-3 border h-100" style="border-left: 4px solid #f59e0b !important;">
      <strong style="color:#d97706;">Operando <span style="font-family:'JetBrains Mono',monospace;">Z&#8322;</span></strong>
      <div class="row g-2 mt-1">
        <div class="col-6">
          <label class="form-label mb-1 small">Real <em>x</em>&#8322;</label>
      ...
      <div id="op2Polar" class="small text-muted mt-2" style="font-family:'JetBrains Mono',monospace;"></div>
    </div>
  </div>
</div>

<!-- Selector de operación -->
<ul class="nav nav-pills mb-3 mt-4 justify-content-center" id="opTab" role="tablist">
  <li class="nav-item"><button class="nav-link active px-3 py-2 fw-bold" data-op="suma" onclick="seleccionarOperacion('suma')">Z&#8321; + Z&#8322;</button></li>
  <li class="nav-item"><button class="nav-link px-3 py-2 fw-bold" data-op="resta" onclick="seleccionarOperacion('resta')">Z&#8321; − Z&#8322;</button></li>
  <li class="nav-item"><button class="nav-link px-3 py-2 fw-bold" data-op="mult" onclick="seleccionarOperacion('mult')">Z&#8321; &times; Z&#8322;</button></li>
  <li class="nav-item"><button class="nav-link px-3 py-2 fw-bold" data-op="div" onclick="seleccionarOperacion('div')">Z&#8321; &divide; Z&#8322;</button></li>
</ul>
  ...
  <div class="col-lg-7 text-center">
    <div class="p-3 bg-white rounded-3 border shadow-sm d-inline-block w-100 h-100 d-flex flex-column justify-content-center align-items-center">
      <div class="d-flex justify-content-center gap-3 w-100 mb-2 px-1 flex-wrap">
        <span class="badge" style="background:#0284c7;">&#9679; Z&#8321;</span>
        <span class="badge" style="background:#f59e0b;">&#9679; Z&#8322;</span>
...
function fmtRect(x, y) {
  const signo = y >= 0 ? '+' : '−';
  return `${x.toFixed(3)} ${signo} j${Math.abs(y).toFixed(3)}`;
}

function dibujarEjes(ctx, w, h, cx, cy) {
...
function dibujarVector(ctx, cx, cy, scale, x, y, color, grosor) {
  const vx = cx + x * scale;
  const vy = cy - y * scale;
...
function dibujarOperacion(x1, y1, x2, y2, xr, yr, op) {
  ...
  dibujarEjes(ctx, w, h, cx, cy);

  // Construcción del paralelogramo (solo tiene sentido geométrico en suma/resta)
  if (op === 'suma' || op === 'resta') {
    ctx.setLineDash([4, 4]);
    ctx.lineWidth = 1.2;
  ...
  dibujarVector(ctx, cx, cy, scale, x1, y1, "#0284c7", 3);
  dibujarVector(ctx, cx, cy, scale, x2, y2, "#f59e0b", 3);
  dibujarVector(ctx, cx, cy, scale, xr, yr, "#8b5cf6", 3.5);
...
function calcularOperacion() {
  ...
  const p1 = aPolar(x1, y1);
  const p2 = aPolar(x2, y2);
  document.getElementById("op1Polar").innerHTML = `Polar: ${p1.r.toFixed(3)} &ang; ${p1.theta.toFixed(2)}&deg;`;
  document.getElementById("op2Polar").innerHTML = `Polar: ${p2.r.toFixed(3)} &ang; ${p2.theta.toFixed(2)}&deg;`;
  ...
  if (operacionActual === 'suma') {
    ...
    pasosHtml = `
      <div class="small text-muted mb-2">
        <strong>Método: forma rectangular (componente a componente)</strong><br>
        <code>x = ${x1.toFixed(2)} + (${x2.toFixed(2)}) = ${xr.toFixed(3)}</code><br>
        <code>y = ${y1.toFixed(2)} + (${y2.toFixed(2)}) = ${yr.toFixed(3)}</code>
      </div>`;
    ...
    pasosHtml = `
      <div class="small text-muted mb-2">
        <strong>Método: forma polar (magnitudes se multiplican, ángulos se suman)</strong><br>
        <code>r = ${p1.r.toFixed(3)} &middot; ${p2.r.toFixed(3)} = ${rr.toFixed(3)}</code><br>
        <code>&theta; = ${p1.theta.toFixed(2)}&deg; + ${p2.theta.toFixed(2)}&deg; = ${th.toFixed(2)}&deg;</code>
      </div>`;
  } else {
    if (p2.r === 0) {
      document.getElementById("resultadoOperacion").innerHTML =
        `<div class="alert alert-danger">No se puede dividir entre Z&#8322; = 0.</div>`;
      return;
    }
    const rr = p1.r / p2.r;
    const th = p1.theta - p2.theta;
    ...
    pasosHtml = `
      <div class="small text-muted mb-2">
        <strong>Método: forma polar (magnitudes se dividen, ángulos se restan)</strong><br>
        <code>r = ${p1.r.toFixed(3)} / ${p2.r.toFixed(3)} = ${rr.toFixed(3)}</code><br>
        <code>&theta; = ${p1.theta.toFixed(2)}&deg; − ${p2.theta.toFixed(2)}&deg; = ${th.toFixed(2)}&deg;</code>
      </div>`;
  }

  const pr = aPolar(xr, yr);
  ...
  document.getElementById("resultadoOperacion").innerHTML = `
    <div class="card p-3 border-0 bg-light h-100 shadow-sm" style="border-left: 4px solid #8b5cf6 !important;">
      ...
      <div class="mb-2">
        ...
        <code class="fs-5 text-dark fw-bold">${fmtRect(xr, yr)}</code>
      </div>
  ...
  dibujarOperacion(x1, y1, x2, y2, xr, yr, operacionActual);
}
