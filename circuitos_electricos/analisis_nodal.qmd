...
$$\mathbf{I}_{ij} = \frac{\mathbf{V}_i - \mathbf{V}_j}{\mathbf{Z}_{ij}} = y_{ij}(\mathbf{V}_i - \mathbf{V}_j)$$
...
4. **Consecuencia posible:** Dependiendo de la topología y de la variable de control, la matriz $[\mathbf{Y}]$ **puede perder la simetría** ($\mathbf{Y}_{ij} \neq \mathbf{Y}_{ji}$).
...
1. **Formulación Matricial:** Calcular las admitancias de rama $\mathbf{Y}_1$, $\mathbf{Y}_2$ y $y_{12}$, y ensamblar directamente por inspección visual el sistema de ecuaciones nodales $[\mathbf{Y}][\mathbf{V}] = [\mathbf{I}]$.
...
* **Admitancias de las ramas:**
  $$\mathbf{Y}_1 = 0.05-j0.05\,\text{S},\qquad
  \mathbf{Y}_2 = 0.05\,\text{S},\qquad
  y_{12}=\frac{1}{Z_{12}}=j0.20\,\text{S}$$

  Aquí $y_{12}$ representa la **admitancia física de la rama** que conecta los nodos 1 y 2.
...
* **Elemento mutuo de la matriz ($\mathbf{Y}_{12}=\mathbf{Y}_{21}$):**
  ...
  **Nota:** $y_{12}$ y $\mathbf{Y}_{12}$ no son la misma magnitud: la primera es la admitancia de la rama y la segunda es el coeficiente fuera de la diagonal de la matriz nodal.
...
El elemento $\mathbf{Y}_{21} = -0.7$ **ya no es igual** a $\mathbf{Y}_{12} = -0.2$. La fuente controlada ha introducido un acoplamiento asimétrico: la tensión del nodo 1 afecta la corriente inyectada al nodo 2, pero no al revés. En este circuito, esta asimetría es consecuencia del acoplamiento introducido por la fuente dependiente.
:::
...
En este circuito particular, los acoplamientos introducidos hacen que la formulación nodal **no resulte simétrica** ($\mathbf{Y}_{ij} \neq \mathbf{Y}_{ji}$). Por ello, el sistema debe resolverse por sustitución directa o mediante una formulación matricial aumentada, en lugar de aplicar únicamente las reglas de inspección de una red pasiva recíproca.
:::
...
El signo negativo de las componentes rectangularas indica que el fasor tiene componentes real e imaginaria negativas respecto de la referencia adoptada. En régimen sinusoidal, esto **no significa que la corriente instantánea cambie de sentido durante una parte del ciclo**; el ángulo $-137.8^\circ$ describe la fase de $\mathbf{I}_o$ respecto del sentido de referencia $B\rightarrow A$.
