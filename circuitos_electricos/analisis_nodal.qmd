...
Las fuentes dependientes pueden introducir acoplamientos asimétricos en el circuito:
...
4. **Consecuencia posible:** La matriz $[\mathbf{Y}]$ puede perder la simetría ($\mathbf{Y}_{ij} \neq \mathbf{Y}_{ji}$), dependiendo de cómo esté conectada y controlada la fuente dependiente.
...
* **Admitancia de la rama entre nodos 1 y 2:**
  $$y_{12}=j0.20\,\text{S}$$
* **Admitancia propia del nodo 1 ($\mathbf{Y}_{11}$):**
  $$\mathbf{Y}_{11}=\mathbf{Y}_1+y_{12}=(0.05-j0.05)+j0.2=\mathbf{0.05+j0.15\,S}$$
* **Admitancia propia del nodo 2 ($\mathbf{Y}_{22}$):**
  $$\mathbf{Y}_{22}=\mathbf{Y}_2+y_{12}=0.05+j0.2=\mathbf{0.05+j0.20\,S}$$
* **Elemento mutuo de la matriz ($\mathbf{Y}_{12}=\mathbf{Y}_{21}$):**
  $$\mathbf{Y}_{12}=-y_{12}=-(j0.2)=\mathbf{-j0.20\,S}$$
...
El valor negativo indica que la corriente real de la VCCS tiene sentido opuesto al sentido de referencia adoptado para $\mathbf{I}_x$. En régimen sinusoidal, el signo de un fasor no debe interpretarse como que la corriente invierte su sentido durante una parte del ciclo; representa un desplazamiento de fase de $180^\circ$ respecto de la referencia.
...
En este ejercicio se adopta $\mathbf{I}_o$ con sentido **de B hacia A**, tal como se define en la ecuación siguiente. Todas las ecuaciones posteriores mantienen esta referencia.
:::
...
El fasor tiene un ángulo de $-137.8^\circ$ respecto al sentido de referencia $B\rightarrow A$. El signo de sus componentes rectangulares no significa que la corriente instantánea cambie de sentido durante una parte del ciclo; describe su fase respecto de la referencia elegida.
...
* Fuente de corriente $6\angle 30^\circ\,\text{A}$ **saliendo** del nodo 2 hacia tierra, de acuerdo con la flecha del esquema. Por tanto, con el convenio de corrientes inyectadas, su contribución al vector de corriente del nodo es $-6\angle30^\circ$ A.
...
$$-6\angle30^\circ = -5.196-j3.0\,\text{A}$$

Término derecho total:

$$\mathbf{I}_{\text{total}} = (-5.196-j3.0) + (2.007-j1.780) = -3.189-j4.780\,\text{A}$$

Finalmente:

$$\mathbf{V} = \frac{-3.189-j4.780}{0.04+j0.0233}$$

$$\boxed{\mathbf{V} \approx -111.49-j54.47\,\text{V}}$$

En forma polar:

$$\boxed{\mathbf{V} \approx 124.08\,\angle\,-153.96^\circ\,\text{V}}$$

**Paso 5 — Verificación por LCK.**

Usando:

$$\mathbf{V}\approx -111.49-j54.47\,\text{V}$$

la suma de las corrientes que salen del nodo 2 por las tres ramas es:

$$
\mathbf{I}_{\text{ramas}}=
\frac{\mathbf{V}-\mathbf{V}_1}{40+j20}
+\frac{\mathbf{V}}{-j30}
+\frac{\mathbf{V}}{50}
$$
...
$$\mathbf{I}_{\text{ramas}}\approx -5.196-j3.000\,\text{A}
=-6\angle30^\circ\,\text{A}$$

Por tanto, se verifica:

$$
\boxed{
\mathbf{I}_{\text{ramas}}+6\angle30^\circ\approx0
}
$$

lo que confirma la LCK y, especialmente, el **signo correcto de la fuente de corriente según la dirección de su flecha**.
...
4. **Convenio de signos de corrientes inyectadas:** Fuentes que entran al nodo suman positivo ($+$); fuentes que salen del nodo aparecen con signo negativo ($-$). Mantén el mismo convenio en todo el ejercicio.
:::
